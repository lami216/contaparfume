...
test("decant sale and bottle purchase live in one invoice workspace", async () => {
  const [app, hub, globals, perfumeCss] = await Promise.all([source("app/conta-app.tsx"), source("app/decant-invoices-page.tsx"), source("app/globals.css"), source("app/perfume-ui-fixes.css")]);
  const css = `${globals}\n${perfumeCss}`;
