...
<<<<<<< /tmp/tmp038ybqb_/ours
    ...
    const legacyCosts = await db.collection("documents").aggregate([
      { $match: { kind: { $in: ["purchase", "decant-purchase"] }, status: "posted" } }, { $sort: { occurredAt: -1 } },
    ...
    const [parties, warehouses, products, productCategories, documents, movements, financialMovements, partyMetricDocuments, partyMetricMovements, paymentAccounts, accountTransfers, productCounter, nextSale, nextPurchase, nextExpense, nextDecantSale, nextDecantPurchase, branding] = await Promise.all([
      ...
      db.collection("documents").find({ kind: { $in: ["sale", "decant-sale", "return", "purchase", "decant-purchase"] } }, { projection: { _id: 0, kind: 1, status: 1, partyId: 1, total: 1, lines: 1 } }).toArray(),
...
<<<<<<< /tmp/tmp038ybqb_/ours
    ...
    const cleanCategories = clean(productCategories).map(category => { const raw = category as Record<string, unknown>; return { id: String(raw.id ?? ""), name: String(raw.name ?? "") }; }).filter(category => category.id && category.name);
...
<<<<<<< /tmp/tmp038ybqb_/ours
    ...
    const bankAccess=hasCapability(principal,"banks.view")||hasCapability(principal,"banks.movements.view"),partyAdmin=hasCapability(principal,"customers.view")||hasCapability(principal,"suppliers.view"),productAdmin=hasCapability(principal,"products.view"),perfumeAccess=hasCapability(principal,"perfume.divisions.view");
    ...
    const allowedDocuments=(clean(documents) as Array<Record<string,unknown>>).filter(document=>hasCapability(principal,"records.view")||(hasCapability(principal,"pos.view")&&document.kind==="sale")||(hasCapability(principal,"purchases.view")&&document.kind==="purchase")||(perfumeAccess&&["decant-sale","decant-purchase"].includes(String(document.kind)))||(hasCapability(principal,"expenses.view")&&document.kind==="expense"));
    ...
    const exposedProducts=productAdmin||perfumeAccess?cleanProducts:(cleanProducts as Array<Record<string,unknown>>).map(({id,name,sku,barcode,piecePrice,wholesalePrice,expiryDate,categoryId,stocks,isArchived})=>({id,name,sku,barcode,piecePrice,wholesalePrice,expiryDate,categoryId,stocks,isArchived,pieceCost:null,lastPurchaseCost:null}));
    ...
    return Response.json({ branding, principal:{principalType:principal.principalType,name:principal.name,permissions:principal.permissions}, parties:exposedParties, warehouses:clean(warehouses), products:exposedProducts, categories:cleanCategories, documents:allowedDocuments, movements:hasCapability(principal,"warehouses.inventory.view")?clean(movements):[], financialMovements:bankAccess?clean(financialMovements):[], partyFinancialSummaries, paymentAccounts:selectorAccounts, accountTransfers:hasCapability(principal,"banks.transfer")?clean(accountTransfers):[], nextProductCode, nextDocumentSequences:{sale:nextSale,purchase:nextPurchase,expense:nextExpense,decantSale:nextDecantSale,decantPurchase:nextDecantPurchase} });
=======
