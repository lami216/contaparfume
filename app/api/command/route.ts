...
export async function POST(request: Request) {
  ...
  try {
    ...
    const map: Record<string, Capability> = {
      ...
      "account-transfer.post":"banks.transfer","account-transfer.update":"banks.transfer.edit","account-transfer.void":"banks.transfer.delete",
      "account-opening-balance-correction.post":"banks.balance_correct","account-opening-balance-correction.update":"banks.balance_correct.edit","account-opening-balance-correction.void":"banks.balance_correct.delete",
      "party.create":body.partyType==="customer"?"customers.create":"suppliers.create","party.update":"customers.edit","party.delete":"customers.delete","party.restore":"customers.edit",
    };
    ...
    if (["party-cash.post","party.update","party.delete","party.restore"].includes(type)) {
      ...
      if (party) {
        ...
        capability = type === "party-cash.post" ? (supplier ? "suppliers.pay" : "customers.collect")
          : type === "party.delete" ? (supplier ? "suppliers.delete" : "customers.delete")
          : (supplier ? "suppliers.edit" : "customers.edit");
      }
    }
    if (["party-cash.update","party-cash.void"].includes(type)) {
      const database = await getDatabase();
      const document = await database.collection("documents").findOne({ id: text(body.documentId), kind: "payment" });
    ...
    if ((type === "product.update" && body.replaceOpeningStock === true) || (type === "product.create" && Number(body.openingStock ?? 0) > 0)) {
      const stockDenied = await requireCapability(request, "warehouses.adjust");
      if (stockDenied) return stockDenied;
    }
