...
<<<<<<< /tmp/tmpqkv550eu/ours
    if(product.perfumeForm==="decant"||product.perfumeForm==="partial"){await db.collection("products").updateOne({id:productId},{$set:{name,barcode,expiryDate:values.expiryDate,note:values.note,categoryId:values.categoryId,piecePrice:values.piecePrice,wholesalePrice:values.wholesalePrice}},{session});return productId;}
