...
function createCloseFlow({dialog,window:windowValue,fetchBackup,writeBackup,approveQuit,onFailure}){
 ...
 async function requestClose(){
  ...
  try{const {response}=await dialog.showMessageBox(windowValue(),{type:'question',title:'الكرنة للعطور',message:'هل تريد إنشاء نسخة احتياطية قبل الخروج؟',buttons:['نعم، إنشاء نسخة','لا، خروج','إلغاء'],defaultId:0,cancelId:2,noLink:true});
   if(response===2)return false;
   if(response===1){approved=true;await approveQuit();return true}
   const saved=await dialog.showSaveDialog(windowValue(),{title:'حفظ النسخة الاحتياطية',defaultPath:backupFilename(),filters:[{name:'نسخة الكرنة للعطور',extensions:['conta.json','json']}]});
