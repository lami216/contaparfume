...
const {spawn}=require('node:child_process');const {join}=require('node:path');const {mkdirSync,createWriteStream}=require('node:fs');
const isolatedUserData=join(app.getPath('appData'),USER_DATA_DIR);mkdirSync(isolatedUserData,{recursive:true});app.setPath('userData',isolatedUserData);const {writeFile,readFile}=require('node:fs/promises');const crypto=require('node:crypto');const net=require('node:net');const {createCloseFlow}=require('./close-flow.cjs');
